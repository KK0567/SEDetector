...
# -----------------------
def stream_ndjson(path: Union[str, Path]) -> Iterator[Dict[str, Any]]:
    path = Path(path)
...
# -----------------------
def compute_process_set_opc(input_path: Union[str, Path], min_actor_count: int = 1) -> set:
    actor_cnt = Counter()
    obj_cnt = Counter()
...
def stream_slices_with_progress_opc(input_path: Union[str, Path], process_set: set, delta_ns: int) -> Iterator[Dict[str, Any]]:
    cur = None
    slice_id = 0
...
# -----------------------
def run(input_path: Union[str, Path], output_path: Union[str, Path], dt_seconds: float,
...
if __name__ == "__main__":
    ...
    ap.add_argument("--in", dest="inp", default="../Result/2.Host_Data/host_0170.json", help="input OpTC NDJSON (.json), one json per line")
    ap.add_argument("--out", dest="outp", default="../Result/3.Slice/host_0170.json", help="output slices jsonl")
