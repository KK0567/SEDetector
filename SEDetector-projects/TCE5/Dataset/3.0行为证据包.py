...
def compress_chain_rle(chain):
    ...
    def flush(b):
        nonlocal mid
        event = b[0]["names"]
        entities = [(x["entity"] or "")[:8] for x in b]
...
def build_evidence_sentence(sl, sem, motifs):
    subj = sl["chain"][0]["entity"][:8]
    obj = sl["chain"][-1]["entity"][:8]
...
if __name__ == "__main__":
    ...
    ap.add_argument("--in", dest="inp", default="fivedirections-3/2.Behavior_Slices/raw_log_slices_labeled_all.json")
    ap.add_argument("--out_jsonl", default="fivedirections-3/3.semantic_mapping/raw_log_slices_labeled_all.json")
    ap.add_argument("--out_txt", default="fivedirections-3/3.semantic_mapping/raw_log_slices_labeled_all.txt")
