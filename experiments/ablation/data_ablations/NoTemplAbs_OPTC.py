...
import run_OPTC as __orig_run
_r = __orig_run
_og = _r.gather_batch_global_feats
def _mg(g, nids, ehids, dev):
    nf, ef = _og(g, nids, ehids, dev)
    nf = nf.clone(); ef = ef.clone()
    nf[:,:,5:69]=0; ef[:,:,0]=0; ef[:,:,3]=0
    ef[:,:,4:132]=0; ef[:,:,132:260]=0
    return nf, ef
...
sys.argv = [
    ...
    "--test_hg",   "../data_OPTC/Hyper_test.json",
            ...
            "--epochs","4","--episodes_per_epoch","60",
...
sys.argv += ["--out_dir", "./outputs_OPTC_abl_NoTemplAbs"] + _orig_argv
