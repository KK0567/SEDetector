...
"""
Raw+HG (随机化 SEU 特征)  |  Dataset: TCE5
...
sys.argv = [
    ...
    "--grad_clip", "1.0",
    "--warmup_ratio", "0.1",
    "--out_dir", "./outputs_TCE5_abl_RawHG",
]
sys.argv += _orig_argv  # re-append --seed etc.

import run_TCE5 as _orig_run

def _randomized_gather(g, node_ids, edge_hids, device):
    import torch
    B, N = node_ids.shape
    _, E = edge_hids.shape
    nf = torch.randn(B, N, g.node_feats.size(1), device=device) * 0.1
    ef = torch.randn(B, E, g.edge_feats.size(1), device=device) * 0.1
    return nf, ef

_orig_run.gather_batch_global_feats = _randomized_gather
_orig_run.main()
